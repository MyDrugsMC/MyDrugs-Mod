...
final class MixingVatRecipeCategory extends AbstractNiceRecipeCategory<MixingVatRecipe> {
    ...
    public void draw(MixingVatRecipe recipe, IRecipeSlotsView slots, GuiGraphics g, double mouseX, double mouseY) {
        ...
        drawPanelLabel(g, uiString("Items"), leftInnerX(), panelY + s(2), leftInnerW() - SLOT - s(8));
        drawPanelLabel(g, uiString("Fluids"), leftX + leftW - SLOT - s(12), panelY + s(2), SLOT + s(10));
        drawBottomInfo(g, jeiString("screen.mydrugs.jei.required_stirs", recipe.requiredStirs()));
        drawTitle(g);
    }
}
