...
import org.mydrugs.mydrugs.client.compat.gas.GasJeiIngredient;
import org.mydrugs.mydrugs.client.compat.gas.GasJeiTypes;
...
public abstract class AbstractNiceRecipeCategory<T> implements AbstractMachineDrawMethods, IRecipeCategory<T> {
    ...
    protected List<Component> tooltip(String... lines) {
        ...
        for (int i = 0; i < lines.length; i++) {
            components[i] = hasLetters(lines[i]) ? ui(lines[i]) : Component.literal(lines[i]);
        }
    ...
    protected List<Component> fluidTankTooltip(String title, Fluid fluid, int amount, int capacity) {
        return tooltip(
                ui(title),
                fluidName(fluid),
                Component.translatable("screen.mydrugs.ui.amount_unit", amount, capacity, "mB")
        );
    }
    ...
    protected List<Component> gasTankTooltip(String title, ResourceLocation gasId, long amount, long capacity) {
        return tooltip(
                ...
                Component.translatable("screen.mydrugs.ui.amount", amount, capacity)
        );
    }
    ...
    protected Component fluidName(Fluid fluid) {
        return fluid == null || fluid == Fluids.EMPTY ? ui("empty") : fluid.getFluidType().getDescription();
    }

    protected Component ui(String label) {
        return Component.translatable(uiKey(label));
    }

    protected String uiString(String label) {
        return ui(label).getString();
    }

    protected String jeiString(String key, Object... args) {
    ...
    private static String uiKey(String label) {
        String key = label.toLowerCase(Locale.ROOT)
                .replace("%", " percent")
                .replaceAll("[^a-z0-9]+", "_")
                .replaceAll("^_+|_+$", "");
        return "screen.mydrugs.ui." + key;
    }
    ...
    protected void drawFrame(GuiGraphics g) {
        ...
        drawCentered(g, jeiString("screen.mydrugs.jei.input"), leftX, s(6), leftW, LABEL_COLOR);
        drawCentered(g, jeiString("screen.mydrugs.jei.output"), rightX, s(6), rightW, LABEL_COLOR);
