package org.mydrugs.mydrugs.client.effects.hallucination;
...
public final class FakeEntityRenderController {
    ...
    private static final int SPAWN_TRIES_PER_TICK = 6;
    private static int pendingSpawnTries;
    ...
    public static void tick() {
        ...
        if (pendingSpawnTries <= 0) {
            if (RANDOM.nextFloat() > (0.015F + AddictionClientState.globalSeverity * 0.035F) * intensityScale) return;
            pendingSpawnTries = MAX_SPAWN_TRIES;
        }

        Camera camera = mc.gameRenderer.getMainCamera();
        int triesThisTick = Math.min(SPAWN_TRIES_PER_TICK, pendingSpawnTries);
        Vec3 spawnPos = findHiddenSpawn(mc, camera, triesThisTick);
        pendingSpawnTries -= triesThisTick;
    ...
    private static Vec3 findHiddenSpawn(Minecraft mc, Camera camera, int tries) {
